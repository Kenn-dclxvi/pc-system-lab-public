...
ただ、このときは同じベンチ中にCPU/cache系のWHEAも1件出ています。Bottom停止との関係は分からないため、この一回だけで常用は決めませんでした。

Frontから入った空気がGPUへ流れる経路ができていれば、Bottomから追加で押し込む空気の寄与はかなり小さいのかもしれません。

だったら止めた方が、騒音、埃、消費電力を少しずつ減らせます。

その後もBottomを止めた状態で試験を続け、CPU設定を変更したあとの長時間FF14まで完走したところで、停止を採用しました。少なくとも現在の構成では、止めたことが大きな問題にはなっていません。
...
高速側ほどLiquidとCPUの温度は低い傾向でした。ただ、試験ごとの開始Liquid温度も違うため、どこまでが回転数の効果かまでは分かりません。

それでも70%主体で長時間試験を行うと、Liquidは45℃で釣り合いました。常時100%にする必要はないと考え、Liquid連動のまま必要な分だけ回すことにしました。
...
実際には、Rearを約700 RPM増やしてもGPU温度はほとんど変わらない。Bottomを止めてもGPU温度はほとんど変わらない。radiator fanは増速時にLiquidとCPUの温度が下がる傾向はありましたが、70%主体でも長時間の熱平衡を保てました。
